...
class main():
	...
	def run(self):
		...
		liste_square = self.board.draw_board(self.couleur_1, self.couleur_2, self.square_size, self.window)# dessin du plateau et met toutes les cases dans une liste
		...
		while self.running:
			...
			for square in liste_square:# pour tous les carrées dans notre liste d'objet carré
				pos_name = square.get_pos_name(mouse[0], mouse[1])# récupère l'id de la case sur laquelle la souris est
				if pos_name != None:
					print(pos_name)
					break
