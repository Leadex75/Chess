...
class ChessBoard():
	...
	def draw_board(self, couleur_1, couleur_2, square_size, window):
		list_square = []
		self.pieces = ChessPieces(window, square_size) #creation des pieces
		lettres = "ABCDEFGH" #lettres pour les colonnes
		number = "87654321" #chiffres pour les lignesœ
		counter = 0
		for row in range(len(lettres)):
			for col in range(len(lettres)):
				...
				square = ChessSquare(color, x, y, square_size, id)# création de la case
				square.draw(window)
